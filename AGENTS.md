...
This document provides essential architectural context, engineering guidelines, safety rules, and development workflows for AI agents and developers working on the **KMSRDP** codebase.
...
### 2.3 KMS / DRM & GPU Capture (`kmsrdp/src/capture/`, `gpu_detile.rs`)
...
### 2.6 Quality, Memory Efficiency & Reliability
* **Explicit, Structured Error Types**:
  * Avoid `anyhow::Result` and stringly-typed errors in core crates (`rdpcore-*`). Define domain-specific `thiserror` enums that callers can match on.
  * Do not return information-free errors such as `Result<T, ()>`.
* **Zero-Allocation Hot Paths (Reuse Buffers)**:
  * Avoid repeated heap allocation on per-frame / per-tile hot paths (`diff.rs`, `encode.rs`, and similar).
  * Prefer stack-resident bitsets (`SmallBitSet` and similar), fixed-size arrays, and reused scratch buffers (`buffer.clear()`).
* **Property-Based Tests and Invariants**:
  * Core protocol decoders (BER, PER, RDP6, FastPath, MCS, and similar) must **never panic** on arbitrary corrupted input, and valid encode/decode must be strictly reversible (round-trip). Cover this with a `proptest!` suite.
* **Secrets and Credentials**:
  * Types that hold passwords, NTLM hashes, or private keys must mask them in `fmt::Debug` (`[REDACTED]`) and must never write plaintext secrets to logs.
* **Structured Logging**:
