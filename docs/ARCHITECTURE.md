...
|---|---|---|---|---|
| RDP6 Planar | Default path. Batches PDUs (Synchronize+Cooperate+Control+FontList) and fragments Confirm Active — handled, code-inspection only | Default path. **CI-confirmed** (`tests/e2e_freerdp.rs`): NLA `+auth-only` plus a short full Planar session with a patterned framebuffer (must stay up; FreeRDP+Xvfb installed in CI) | Default path (FreeRDP-based backend) | Not used — falls back to NSCodec |
...
| GFX / AVC420 | **Off by default** (`KMSRDP_GFX=1` opt-in) — mstsc can disconnect on GFX protocol errors; treat as unverified/risky, not confirmed working | Capability-negotiated when `KMSRDP_GFX=1`. **Automated**: oversized WireToSurface → `RDP_SEGMENTED_DATA` MULTIPART (`0xe1`) regression in `rdpcore-rdpegfx`; Caps→Start/Wire/End via `e2e_mock_client` (`cargo test -p rdpcore-server --features gfx`). Optional FreeRDP `/gfx:AVC420` smoke (soft-skip if client rejects). **Manual**: xfreerdp3 hang on large IDR fixed by multipart (2026-09-08); re-check with [HARDWARE_CHECKLIST.md](HARDWARE_CHECKLIST.md) | Follow checklist under GFX; not separately CI-confirmed | Not applicable (NSCodec path) |
...
| Known real-world bug (fixed) | — | **xfreerdp3 + `KMSRDP_GFX=1`**: SINGLE segment >64KiB → `zgfx_decompress failure` / disconnect under heavy scroll; fixed by MULTIPART wrap (v0.1.65+) | **Confirmed on real GTX 1080 hardware, 2026-08-24**: bulk-queue backpressure during large legacy-bitmap updates silently truncated mid-frame and killed the session (looked like "bottom half never renders"); fixed and re-verified live on the reporting machine the same day | — |
| Overall confirmation level | Code-inspection only (extensive wire-quirk handling in `rdpcore-connector`, no automated e2e) | **Test-confirmed** (NLA + Planar short session in CI) + GFX wire/mock regressions; real GFX pixel QA via checklist | **Real-hardware confirmed** (backpressure fix) + checklist for GFX | README claims real-hardware tested; no automated test |

**Gaps**: pixel-exact client framebuffer compare is not in CI yet (session-up + mock decode instead); mstsc GFX remains unverified; Guacamole GFX not in CI; see [HARDWARE_CHECKLIST.md](HARDWARE_CHECKLIST.md) for manual Planar/GFX scroll and residual-tile checks.
