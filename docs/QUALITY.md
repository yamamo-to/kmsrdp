...
| Maintainability | 8/10 | Crate split and `docs/AGENTS.md` rules are clear. Large files such as `server.rs` / `capture.rs` and the cost of learning a from-scratch protocol stack remain. |
| Documentation | 7/10 | README and `docs/SECURITY.md` cover limits and deployment. rustdoc, architecture diagrams, and a client interoperability matrix are thin. |
...
| Documentation | 7/10 | `docs/ARCHITECTURE.md` (module map + Mermaid sequence diagram) exists and is reasonably current. Client interoperability matrix and a README/`docs/SECURITY.md` freshness pass are in progress as a follow-up. |
